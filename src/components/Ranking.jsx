    ...
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {[["stock", "📈 주가 순위"], ["assets", "💰 총자산 순위"]].map(([m, l]) => (
          <button key={m} onClick={() => setMode(m)} style={{ padding: isMobile ? "8px 14px" : "10px 20px", background: mode === m ? "rgba(100,130,255,0.15)" : "rgba(100,130,255,0.04)", border: `1px solid ${mode === m ? "rgba(120,160,255,0.3)" : "rgba(120,140,255,0.1)"}`, borderRadius: 10, color: mode === m ? "#9cb8ff" : "#778", cursor: "pointer", fontSize: isMobile ? 13 : 14, fontWeight: mode === m ? 600 : 400 }}>{l}</button>
        ))}
      </div>
      ...
      {isMobile ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
          {sorted.slice(0, 3).map((s, i) => (
            <Card key={s.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", background: i === 0 ? "linear-gradient(160deg, rgba(255,215,0,0.08), rgba(255,180,0,0.04))" : undefined, borderColor: i === 0 ? "rgba(255,215,0,0.2)" : undefined }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ fontSize: 22 }}>{medals[i]}</div>
                <div style={{ fontSize: 28 }}>{s.emoji}</div>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14, color: "#e0e6ff" }}>{s.company}</div>
                  <div style={{ fontSize: 11, color: "#778" }}>{s.name}</div>
                </div>
              </div>
              <div style={{ fontSize: 18, fontWeight: 800, color: i === 0 ? "#ffd740" : "#9cb8ff" }}>{getVal(s).toLocaleString()}원</div>
            </Card>
          ))}
        </div>
      ) : (
      ...
      {sorted.slice(3).map((s, i) => (
        <Card key={s.id} style={{ marginBottom: 6, display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "10px 12px" : "12px 16px" }}>
          ...
          <div style={{ fontSize: isMobile ? 14 : 16, fontWeight: 700 }}>{getVal(s).toLocaleString()}원</div>
        </Card>
      ))}
        ...
        <div style={{ height: isMobile ? 200 : 240 }}>
          <ResponsiveContainer>
            <BarChart data={sorted.map(s => ({ name: s.name, value: getVal(s) }))} layout="vertical">
              ...
              <YAxis dataKey="name" type="category" tick={{ fill: "#aab", fontSize: isMobile ? 11 : 12 }} width={isMobile ? 40 : 48} />
