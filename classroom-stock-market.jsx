...
export default function App() {
  ...
  const save = useCallback(async (s, t, w, m, e, tl) => {
    try {
      await window.storage.set("s42-students", JSON.stringify(s));
      await window.storage.set("s42-txs", JSON.stringify(t));
      await window.storage.set("s42-meta", JSON.stringify({ w, m }));
      await window.storage.set("s42-evts", JSON.stringify(e));
      if (tl !== undefined) await window.storage.set("s42-tickets", JSON.stringify(tl));
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("s42-students");
        if (r?.value) {
          setStudents(JSON.parse(r.value));
          const t = await window.storage.get("s42-txs");
          setTxs(t?.value ? JSON.parse(t.value) : []);
          const m = await window.storage.get("s42-meta");
          if (m?.value) { const d = JSON.parse(m.value); setWeek(d.w || 1); setMission(d.m || ""); }
          const e = await window.storage.get("s42-evts");
          setEvts(e?.value ? JSON.parse(e.value) : []);
          const tl = await window.storage.get("s42-tickets");
          setTicketLog(tl?.value ? JSON.parse(tl.value) : []);
        } else { initSample(); }
      } catch { initSample(); }
      setLoading(false);
    })();
  ...
  const saveTickets = useCallback((tl) => {
    setTicketLog(tl);
    try { window.storage.set("s42-tickets", JSON.stringify(tl)); } catch {}
  }, []);
